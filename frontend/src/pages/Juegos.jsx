...
export default function Juegos() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingJuego) await updateJuego(editingJuego.id, formData);
      else await createJuego(formData);
      loadJuegos();
      closeModal();
    } catch (error) {
  ...
  const closeModal = () => {
    ...
    setFormData({
      nombre: '', genero: '', plataforma: '', horas_jugadas: 0,
      estado: 'pendiente', fecha_inicio: '', fecha_fin: '',
      porcentaje_completado: 0, platinado: false, logros: '', comentario_personal: ''
    });
  };
  ...
  return (
    <PageWrapper>
      <header className="page-header">
        ...
        <button className="add-btn" onClick={() => setShowModal(true)}>+ Agregar Juego</button>
      </header>
      ...
      {loading ? (
        ...
        <EmptyState>
          ...
          <p>Agrega tu primer juego para empezar tu legado.</p>
        </EmptyState>
      ) : (
        <Grid>
          {juegos.map(juego => (
            <Card key={juego.id}>
              ...
              <Footer>
                <Badge data-status={juego.estado}>{juego.estado}</Badge>
        ...
        <Overlay onClick={closeModal}>
          <Modal onClick={e => e.stopPropagation()}>
            ...
            <Form onSubmit={handleSubmit}>
              ...
              <InputRow>
                <Input name="genero" placeholder="Género" value={formData.genero} onChange={handleChange} required />
                <Input name="plataforma" placeholder="Plataforma" value={formData.plataforma} onChange={handleChange} required />
              </InputRow>
              <InputRow>
                <Input type="number" name="horas_jugadas" placeholder="Horas" value={formData.horas_jugadas} onChange={handleChange} />
              ...
              <label>{formData.porcentaje_completado}%</label>
              <input type="range" name="porcentaje_completado" min="0" max="100" value={formData.porcentaje_completado} onChange={handleChange} />
              <CheckLabel><input type="checkbox" name="platinado" checked={formData.platinado} onChange={handleChange} /> Platinado 🏆</CheckLabel>
              <ModalBtns>
...
const Modal = styled.div`
  ...
  h2 { color: #007bff; margin: 0 0 20px; }
`
